...
> 本体、OD、口径、关键词、三个 Agent 模式、三级召回,以及三条硬不变量。

---

这是本体系统的「名词表」。你不需要全懂,但**至少要懂 OD、口径、关键词这三个**,因为日常纠错就是在这三处操作。

> **术语统一说明**:「**口径(Metric)**」是把自然语言桥接到确定查询的那个立法单元(表 `lakehouse_metric`)。早期材料和部分代码里也把它叫「**意图 / Intent**」(旧表 `lakehouse_metric_intent`)——同一个概念。本文一律使用「口径」。
...
| **口径 / Metric** | 口径 | `lakehouse_metric` | 把「自然语言的模糊」桥接到「SQL 的确定」 | ★★ 重点 |
...
- **口径 和 关键词**:**口径**是一个查询模板(锚定在某个 OD 上,包含 度量 / 过滤 / 自动分组 / 透视配置);**关键词**是触发词(指向某个 Property,或指向某个口径)。两者合起来把「人话」变成「确定的查询」。
...
| 用哪种查询形态 | LLM | 从绑定到这些 OD 的口径里挑(有限集选择) |
...
系统里跑三个**互相独立**的 Agent 模式,由线程上的 `agent_type` 区分(一旦设定不可改):

| 模式 | 中文 | 干什么 |
|---|---|---|
| **lakehouse** | 湖仓(查询)模式 | 自然语言 → 召回 → 选口径填参 → SmartQuery → 答案。日常提问、以及[数据集测试](/zh/docs/question-sets/)重放的,都是这个模式 |
| **builder** | 构建(建模)模式 | 访谈式地创建 OD / 口径 / Link,产出是 `pending` 提案,**人工激活**后才生效 |
| **explore** | 探索(草拟口径)模式 | 进阶:对话式地和你一起把一个新口径**草拟**出来——LLM 产出的是结构化的 `commit_card`(度量 + 维度 + 过滤),**而不是 SQL**,再交给引擎编译 |

> 三个模式共一条铁律:**LLM 永远只是从有限集里「选结构、填参数」,可执行 SQL 由确定性引擎拼。** builder 和 explore 都只产「提案 / 草稿」,落库要人点一下激活。
>
> **实践提示**:builder 模式当前**还不够可靠**,建模建议**优先手动**(见 [第 2 步](/zh/docs/builder-mode/));explore 属于进阶玩法,日常先用不到。
